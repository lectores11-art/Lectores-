...
export function CalendarPageClient({
  ...
  async function createEvent(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const timeZoneInput = String(form.get("timeZone") || CALENDAR_DAY_ZONE);
      const zone = isHubZone(timeZoneInput) ? timeZoneInput : CALENDAR_DAY_ZONE;
      const startsRaw = String(form.get("startsAt") || "").trim();
      const endsRaw = String(form.get("endsAt") || "").trim();
      const startsAt = wallTimeToUtc(startsRaw, zone);
      const endsAt = endsRaw ? wallTimeToUtc(endsRaw, zone) : null;
  ...
  const eventsByDay = useMemo(() => {
    ...
    for (const event of visibleEvents) {
      const key = dayKeyInZone(new Date(event.starts_at), CALENDAR_DAY_ZONE);
      ...
      <div className="mx-auto flex w-full max-w-[1080px] flex-1 flex-col">
        <div className="relative mb-3 grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2">
          ...
          <div className="flex flex-col items-center">
            ...
            <p className="min-h-[18px] text-center text-[12px] text-[#8a8a8a] sm:text-[13px]">
              {now ? formatHubClockLine(now) : ""}
            </p>
          </div>
        ...
        {showForm && isAdmin ? (
          ...
          >
            ...
            <div className="space-y-2 sm:col-span-2">
              <Label>Esta hora está en</Label>
              <select
                name="timeZone"
                defaultValue={CALENDAR_DAY_ZONE}
              ...
              <p className="text-[13px] text-[#8a8a8a]">
                El evento se muestra siempre en España, Argentina y México. Elegí
                en cuál de las tres estás escribiendo la hora.
              </p>
            </div>
            <div className="space-y-2">
              <Label>Tipo</Label>
                    ...
                    <div className="space-y-0.5">
                      {dayEvents.map((event) => (
                        <button
                          ...
                          className="block w-full text-left text-[11px] font-medium leading-4 text-[#2B6BF2] hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2B6BF2]"
                        >
                          <span className="block truncate">
                            {formatHubChipLine(new Date(event.starts_at))}
                          </span>
                    ...
                    <div className="space-y-1">
                      {group.events.map((event) => (
                        <button
                          ...
                          type="button"
                          onClick={() => setSelectedEvent(event)}
                          className="flex w-full flex-col gap-0.5 rounded-md px-1 py-1.5 text-left hover:bg-[#f7f7f7] sm:flex-row sm:items-baseline sm:gap-3"
                        >
                          <span className="text-[12px] text-[#5f5f5f] sm:w-56 sm:shrink-0">
                            {formatHubTimes(new Date(event.starts_at))
